...
function buildHelpMessage(lang: 'ar' | 'en'): string {
  if (lang === 'en') {
    return [
      ...
      'Note: passport photos are always processed automatically and immediately — this toggle only controls the detailed reply text:',
      '• "تفعيل الاستخراج التلقائي" — enable (send a detailed reply after each successful extraction)',
      '• "الغاء الاستخراج التلقائي" — disable (default: extraction still happens automatically, just without a detailed reply here)',
      '',
  ...
  return [
    ...
    'ملاحظة: استخراج بيانات الجواز يعمل دائماً وفوراً تلقائياً — هذا التبديل يتحكم فقط بإرسال رد تفصيلي بعد كل استخراج ناجح:',
    '• "تفعيل الاستخراج التلقائي" — تفعيل (إرسال رد تفصيلي بعد كل استخراج ناجح)',
    '• "الغاء الاستخراج التلقائي" — إلغاء (الافتراضي: الاستخراج يستمر تلقائياً، فقط بدون رد تفصيلي هنا)',
    '',
